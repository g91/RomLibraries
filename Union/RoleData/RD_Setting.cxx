...
int RoleDataEx::CheckIntOverflow(INT64 currentValue, INT64 addValue)
{
	INT64 tmpValue = currentValue + addValue;
	if (tmpValue >= INT_MAX)
	{
		return INT_MAX;
	}
...
void	RoleDataEx::SetValue(RoleValueName_ENUM Type, double Value, RoleDataEx* WriteRole)
{
	...
	{
		int newValue = CheckIntOverflow(Value, 0);
		//if (TempData.Attr.Ability->TotalTpExp == (int)Value)
		//	return;
		//TempData.Attr.Ability->TotalTpExp = (int)Value;
		if (TempData.Attr.Ability->TotalTpExp == newValue)
			return;
		TempData.Attr.Ability->TotalTpExp = newValue;
	}
	break;
	case EM_RoleValue_TpExp:
	{
		...
		//	TempData.Attr.Ability->TotalTpExp = CheckIntOverflow(TempData.Attr.Ability->TotalTpExp, DTpExp);
		//}
		//TempData.Attr.Ability->TpExp = (int)Value;

		if (TempData.Attr.Ability->TpExp == newValue)
			return;
		int DTpExp = newValue - TempData.Attr.Ability->TpExp;
		if (DTpExp > 0)
		{
			//TempData.Attr.Ability->TotalTpExp += DTpExp;
			TempData.Attr.Ability->TotalTpExp = CheckIntOverflow(TempData.Attr.Ability->TotalTpExp, DTpExp);
		}
		TempData.Attr.Ability->TpExp = newValue;
	}
	break;
	...
	{
		PlayerBaseData->Defendant.MoneyTraderCount = (float)int(Value);
	} break;
...
void    RoleDataEx::AddValue(RoleValueName_ENUM Type, double  Value, RoleDataEx* WriteRole)
{
	...
	case EM_RoleValue_AddTP:
	{
		Value = CheckIntOverflow(TempData.Attr.Ability->AddTp, Value);
		TempData.Attr.Ability->AddTp = Value;
	}
	break;
	case EM_RoleValue_CultivatePet_Money:
	...
	case EM_RoleValue_TpExp:
	{
		...
		PlayerTempData->Log.Exp_TP = CheckIntOverflow(PlayerTempData->Log.Exp_TP, Value);
		TempData.Attr.Ability->TotalTpExp = CheckIntOverflow(TempData.Attr.Ability->TotalTpExp, Value);
		Value = CheckIntOverflow(TempData.Attr.Ability->TpExp, Value);
		TempData.Attr.Ability->TpExp = Value;
	}
	break;
	case EM_RoleValue_DebtExp:		//經驗值負債
...
void    RoleDataEx::SetInitNPC(GameObjDbStructEx* TempDB)
{
	...
	if (TempData.Attr.Ability->Level > 200)
		TempData.Attr.Ability->Level = 200;
	/*
		if( TempDB->NPC.LevelRange > 1 )
