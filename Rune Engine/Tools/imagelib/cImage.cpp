...
bool cImage::Alpha1Bit()
{
	if (m_depth != 32)
		return false;

	DWORD *pColor = (DWORD*)m_pPixels;
	for ( int y = 0; y < m_height; y++ )
	{
		for ( int x = 0; x < m_width; x++ )
		{
			if ( pColor[0] > 0x7FFFFFFF )
				pColor[0] |= 0xFF000000;
			else
				pColor[0] = 0x00FFFFFF;
			pColor++;
	...
	if ( newImg.Create( m_width, m_height, 32, m_imageFormat ) ) {
		...
		for ( int yy = 0; yy < m_height; yy++ ) {
			for ( int xx = 0; xx < m_width; xx++ ) {
				...
				// 是否有透明值
				if ( !boTransparent && c.rgbReserved > 255 )
					boTransparent = true;
...
bool cImage::Load( const TCHAR *fileNameStr, ImageLibFormat format )
{
	...
	{
		tagRGBQUAD* pixel = (tagRGBQUAD*)m_pPixels;
		for ( int i = 0; i < m_width * m_height; i++ )
		{
			if ( pixel[i].rgbReserved > 255 )
			{
				SetTransparent( true );
				break;
			}
...
bool cImage::Save( const TCHAR *fileNameStr, ImageLibFormat format )
{
	FILE *fptr;
	bool retBool;
	if ( (fptr = fopen( fileNameStr, "wb" )) == NULL ) {
		return false;
	}
