...
void    RoleDataEx::ReCalculatePhantom()
{
	...
	for( int i = 0 ; i < 2 ; i++ )
	{
		...
		for( int i = 0 ; i < Max_WearEqType_Count ; i++ )
		{
			...
			float value	= (float)objDB->Phantom.abi_Phantom.EqTypeValue[i];
			...
			TempData.Attr.Mid.BodyRate[ type ] = CalRate( TempData.Attr.Mid.BodyRate[type ] , value );
		}
	}
}
	...
	if( IsNPC() )
	{
		...
		if( Target->TempData.Attr.Effect.AI_LowAttackPriority == false && HatePoint > 1 )
		{
			...
			if( NPCAtkTarget != NULL && AttackTarget != NULL && AttackTarget->TempData.Attr.Effect.AI_LowAttackPriority == false )
			{
				...
				if( Length( Att ) >= 100 )
				{
					MaxHitPoint = int(NPCAtkTarget->TotalHatePoint() * 13 / 10);
				}
				else
				{
					MaxHitPoint = int(NPCAtkTarget->TotalHatePoint() * 11 / 10);
				}
			}
			...
			if( PetOwner == NULL || IsPet( PetOwner ) == false )
			{
				...
				{
					Target->TempData.NPCHate.MaxHitPoint = (int)NPCHate->TotalHatePoint();
...
bool	RoleDataEx::CalTempHate( RoleDataEx* Att , int Hate )
{
	...
	if( Hate == -1 )
	{
		Hate = int(TempData.NPCHate.MaxHitPoint - NPCHate->HatePoint + 10);
	}
...
int    RoleDataEx::CliAddBuf(  int MagicOwnerID , int MagicID , int MagicLv , bool IsPK , int EffectTime  )
{
	...
	{
		Buf.Point = int(( TempData.Attr.Fin.MDMG * FixArg().Dmg_HealArg + 80 ) * Buf.Point / 100);
	}
		...
		if( MagBuf->GUID == OrgDB->GUID )
		{
			...
			if( MagBuf->MagicBase.TempHateCost != 0 )
			{
				...
				if( HateInfo != NULL )
				{
					...
					TempData.NPCHate.MaxHitPoint = (int)HateInfo->TotalHatePoint();
				}
			}			
			break;
...
bool	RoleDataEx::ReCalhateList( )
{
	...
	for( int i = 0 ; i < TempData.NPCHate.Hate.Size() ; i++ )
	{
		...
		int TotalHatePoint = (int)TempData.NPCHate.Hate[i].TotalHatePoint();
...
float RoleDataEx::CalRate(float OrgSpeed, float AddSpeed)
{
