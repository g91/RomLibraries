...
// You can turn this on, but everything slows to a real crawl in debug mode
#define EXCESSIVE_PARANOIA 0

#if EXCESSIVE_PARANOIA && defined(DEBUG)
#define PARANOID_CURVE_CHECK(curve) Assert ( CurveFormatIsInitializedCorrectly ( curve ) )
#else
...
static real32 const QuaternionCurveScaleOffsetTable[16][2] =
{
...
bool GRANNY
CurveFormatIsInitializedCorrectly(curve2 const &Curve)
{
...
BeginCurveCopyInPlace(curve_builder *Builder, curve2 const &SourceCurve)
{
    if ( Builder != NULL )
    {
        Assert ( CurveFormatIsInitializedCorrectly ( SourceCurve ) );
        BeginCurveInPlace (
            Builder,
                            ...
                            void *Memory, bool AllocateCurveMemory)
{
    ...
    if ( EndAggrPlacement ( Allocator, Memory ) )
    {
        ...
        Assert ( CurveFormatIsInitializedCorrectly ( *Curve ) );
                   ...
                   bool RescaleResults)
{
    ...
    if (RescaleResults)
    {
        ...
        real32 MinDot = Real32AlmostInfinity;
        real32 MaxDot = -Real32AlmostInfinity;
        {for(int Control = 0; Control < NumControls; ++Control)
        {
            real32 const* CurControl = ControlSrc + Control * ControlStrideInDwords;
...
#define IMPL_CURVE_FIND_KNOT_Quantized(MIXED_NAME, Bits)                                            \
    ...
    {                                                                                               \
        ...
        int32x KnotIndex = FindKnotUint ## Bits ( CurveKnotCount, GET_KNOT_POINTER, (knot_type)QuantisedT ); \
        if ( KnotIndex == CurveKnotCount - 1 )                                                      \
        {                                                                                           \
        }                                                                                           \
        else                                                                                        \
...
{
    PARANOID_CURVE_CHECK ( Curve );
    Assert ( CurveFormatIsInitializedCorrectly ( Source ) );
    CURVE_DATA_TYPE &CurveData = *(CURVE_DATA_TYPE*) Curve.CurveData.Object;
                                        ...
                                        real32 const* NOALIAS IdentityVector )
{
    ...
    {for ( int32x Dim = 0; Dim < 4; Dim++ )
    {
        ...
        ControlScales [Dim] = QuaternionCurveScaleOffsetTable[TableIndex][0] * (1.0f / ((real32)((1<<15)-1)));
        ControlOffsets[Dim] = QuaternionCurveScaleOffsetTable[TableIndex][1];
    }}
                                        ...
                                        real32 const *KnotSrc, real32 const *ControlSrc )
{
    ...
    {for ( int32 Dim = 0; Dim < 4; Dim++ )
    {
        ...
        else
        {
            ...
            {
                real32 Min = QuaternionCurveScaleOffsetTable[TableEntry][1];
                real32 Max = QuaternionCurveScaleOffsetTable[TableEntry][0] + Min;
            ...
            ControlScales[Dim] = QuaternionCurveScaleOffsetTable[ScaleOffsetTableEntry][0] / ((real32)((1<<15)-1));
            ControlOffsets[Dim] = QuaternionCurveScaleOffsetTable[ScaleOffsetTableEntry][1];
        }
    }}
                                        ...
                                        real32 const* NOALIAS IdentityVector )
{
    ...
    {for ( int32x Dim = 0; Dim < 4; Dim++ )
    {
        int32x TableIndex = ScaleOffsetTableEntry & 0xf;
        ScaleOffsetTableEntry >>= 4;
        ControlScales [Dim] = QuaternionCurveScaleOffsetTable[TableIndex][0] * (1.0f / ((real32)((1<<7)-1)));
        ControlOffsets[Dim] = QuaternionCurveScaleOffsetTable[TableIndex][1];
    }}
                                        ...
                                        real32 const *KnotSrc, real32 const *ControlSrc )
{
    ...
    {for ( int32 Dim = 0; Dim < 4; Dim++ )
    {
        ...
        else
        {
            ...
            CurveData.ScaleOffsetTableEntries |= ( ScaleOffsetTableEntry << ( Dim * 4 ) );
            ControlScales[Dim] = QuaternionCurveScaleOffsetTable[ScaleOffsetTableEntry][0] / ((real32)((1<<7)-1));
            ControlOffsets[Dim] = QuaternionCurveScaleOffsetTable[ScaleOffsetTableEntry][1];
        }
    }}
...
// #define MIXED_NAME D4nI1K16uC16u
// #define LOWER_NAME d4ni1_k16u_c16u
...
// IMPL_CURVE_FIND_KNOT_Quantized(MIXED_NAME, 16);
// IMPL_CURVE_FIND_CLOSE_KNOT_Quantized(MIXED_NAME, 16);
// // IMPL_CURVE_TRANSFORM_SAMPLES_HINGE(MIXED_NAME);
// // IMPL_CURVE_NOTE_SAMPLE_TRANSFORM_HINGE(MIXED_NAME);
...
//  /* Compute the base transformation first (same as below!) */
//  real32 InverseFirstControl[4];
//  real32 RotAxis[3];
//  {
//          uint16 Quant[4];
...
//      // This doesn't change when we twiddle the rotation later
//      ComputeBestRotationAxis(SampleCount, Samples, InverseFirstControl, RotAxis);
//          QuantizeVector(RotAxis, Quant, 3);
//          ExtractQuantizedVector(Quant, RotAxis, 4);

//      real32 ThetaMin = Real32AlmostInfinity;
//      real32 ThetaMax = -Real32AlmostInfinity;
//      {for (int Sample = 0; Sample < SampleCount; Sample++)
//      {
//          real32 XFormed[4];
//          real32 *CurSample = Samples + (Sample * Dimension);
//          QuaternionMultiply4(XFormed, CurSample, BaseQuat);

//          real32 CosThetaOver2 = CurSample[3];
//          real32 SinThetaOver2 = InnerProduct3(CurSample, RotAxis);
//          real32 ThetaOver2    = (real32)IntrinsicATan2(SinThetaOver2, CosThetaOver2);

//          ThetaMin = Minimum(ThetaMin, ThetaOver2);
//          ThetaMax = Maximum(ThetaMax, ThetaOver2);
//      }}

//      // We need to adjust the base quat such that the samples lie
//      // symmetrically about the origin
//      real32 const ThetaMid = (ThetaMin + ThetaMax) * 0.5f;
//      real32 Adjustment[4];
//      Adjustment[0] = RotAxis[0] * (real32)IntrinsicSin(ThetaMid);
//      Adjustment[1] = RotAxis[1] * (real32)IntrinsicSin(ThetaMid);
//      Adjustment[2] = RotAxis[2] * (real32)IntrinsicSin(ThetaMid);
//      Adjustment[3] = (real32)IntrinsicCos(ThetaMid);
//      QuaternionMultiply4(BaseQuat, Adjustment, BaseQuat);
...
//  /* Compute the base transformation first (same as above!) */
//  real32 RotAxis[3];
//  {
//      real32 BaseQuat[4];
//      Copy32(4, Samples, BaseQuat);
...
//      ComputeBestRotationAxis(SampleCount, Samples, InverseFirstControl, RotAxis);
//      QuantizeVector(RotAxis, CurveData.RotationAxis, 3);
//          ExtractQuantizedVector(CurveData.RotationAxis, RotAxis, 3);
...
//      {
//          real32 XFormed[4];
//          real32 const *CurSample = Samples + (Sample * Dimension);
//          QuaternionMultiply4(XFormed, CurSample, BaseQuat);
...
//          /* Encode, and reextract the quaternion */
//      QuantizeVector(BaseQuat, CurveData.BaseQuaternion, 4);
//          ExtractQuantizedVector(CurveData.BaseQuaternion, BaseQuat, 4);
//  }
// }
...
// void GRANNY
// CURVE_EXTRACT_KNOT_VALUES(MIXED_NAME)(curve2 const &Curve,
//                                    int32x KnotIndex, int32x KnotCount,
...
//      // Set up the scale and offset
//      const int MaxControlValue = (1 << 16) - 1;

//      real32 ValueScale = (CurveData.ValueRange / float(UInt16Maximum)) * 3;  // [0, 3]
//      ValueScale *= (1.0f / MaxControlValue);                         // adjust for the scale in the variable

//      {for ( int32x Count = 0; Count < KnotCount; Count++ )
//      {
//          // The control is sin(theta/2)
//          real32 const SinVal = CurveData.ValueStart + (*CurControl * ValueScale);

//          real32 CosVal;
//          {
//              real32 CosSq = 1.0f - SinVal * SinVal;
//              if (CosSq < 0.0f)
//              {
//                  CosVal = 0.0f;
//              }
//              else
//              {
//                  CosVal = SquareRoot(CosSq);
//              }
//          }

//          ControlResults[0] = RotAxis[0] * SinVal;
//          ControlResults[1] = RotAxis[1] * SinVal;
//          ControlResults[2] = RotAxis[2] * SinVal;
//          ControlResults[3] = CosVal;
//          QuaternionMultiply4(ControlResults, ControlResults, BaseQuat);
...
//  float ValueMin = Real32AlmostInfinity;
//  float ValueMax = -Real32AlmostInfinity;
//  {for (int32x Index = 0; Index < KnotCount; Index++)
//  {
//      real32 const *CurSrcControl = ControlSrc + Index*4;

//      // SinTheta is the last coord...
//      real32 SinTheta = CurSrcControl[3];

//      ValueMin = Minimum(ValueMin, SinTheta);
//      ValueMax = Maximum(ValueMax, SinTheta);
//  }}

//  const int MaxControlValue = (1 << 16) - 1;

//  // Set up the scale
//  real32 ControlMultiplier = 0.0f;
//  {
//      real32 const Range = (ValueMax - ValueMin);
//      int32x const ScaledValue = RoundReal32ToInt32((Range / 3.0f) * UInt16Maximum + 0.5f);

//      Assert(ScaledValue >= 0 && ScaledValue <= UInt16Maximum);
//      CurveData.ValueRange = (uint16)ScaledValue;
//      CurveData.ValueStart = ValueMin;

//      //--- todo: epsilon!
//      if (Range > 1e-7)
//      {
//          ControlMultiplier = MaxControlValue / Range;
//      }
//  }
...
//      // SinTheta is the last coord...
//      real32 SinTheta = CurSrcControl[3];
//      real32 Scaled = (SinTheta - ValueMin) * ControlMultiplier;

//      if (Scaled < 0.0f)
//      {
//          Scaled = 0.0f;
//      }
//      else if ( Scaled > real32(MaxControlValue) )
//      {
//          Scaled = real32(MaxControlValue);
//      }

//      int32x resultControl = RoundReal32ToInt32 ( Scaled );
//      Assert ( resultControl >= 0 && resultControl <= MaxControlValue );
//      FirstControl[Index] = (uint16)resultControl;
//  }}
// }
...
//  Assert ( CurveData.CurveDataHeader.Degree == CurveDataSource.CurveDataHeader.Degree );
//  CurveData.OneOverKnotScaleTrunc  = CurveDataSource.OneOverKnotScaleTrunc;
//  Copy( SizeOf(CurveDataSource.BaseQuaternion), CurveDataSource.BaseQuaternion, CurveData.BaseQuaternion );
//  Copy( SizeOf(CurveDataSource.RotationAxis), CurveDataSource.RotationAxis, CurveData.RotationAxis );
//  CurveData.ValueRange = CurveDataSource.ValueRange;
//  CurveData.ValueStart = CurveDataSource.ValueStart;
//  Assert ( CurveData.KnotControlCount == CurveDataSource.KnotControlCount );
//  Copy ( CurveData.KnotControlCount * sizeof ( *CurveData.KnotsControls ), CurveDataSource.KnotsControls, CurveData.KnotsControls );
// }

// void GRANNY
// CURVE_SCALE_OFFSET_SWIZZLE(MIXED_NAME) ( curve2 &Curve, int32x Dimension,
//                                       real32 const *Scales, real32 const *Offsets,
//                                       int32x const *Swizzles )
// {
//  PARANOID_CURVE_CHECK ( Curve );
...
//  // We just need to twiddle the rotation axis and the base
//  // quaternion
//  real32 BaseQuat[4];
//  real32 RotAxis[3];
//  ExtractQuantizedVector(CurveData.BaseQuaternion, BaseQuat, 4);
...
//  {for(int Dim = 0; Dim < 3; Dim++)
//  {
//      RotAxis[Dim] = TempAxis[Swizzles[Dim]] * Scales[Dim];
//  }}

//  // and the base quaternion
//  TempAxis[0] = BaseQuat[0];
//  TempAxis[1] = BaseQuat[1];
//  TempAxis[2] = BaseQuat[2];
//  {for(int Dim = 0; Dim < 3; Dim++)
//  {
//      BaseQuat[Dim] = TempAxis[Swizzles[Dim]] * Scales[Dim];
//  }}

//  QuantizeVector(BaseQuat, CurveData.BaseQuaternion, 4);
//  QuantizeVector(RotAxis, CurveData.RotationAxis, 3);

//  // we don't need to twiddle the lower bits of the control,
//  // everything should neighborhood as before.
// }
...
// #define MIXED_NAME D4nI1K8uC8u
// #define LOWER_NAME d4ni1_k8u_c8u
...
//      // Set up the scale and offset
//      const int MaxControlValue = (1 << 8) - 1;
