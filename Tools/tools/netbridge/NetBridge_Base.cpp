...
void	CNetBridge_Base::Flush ()
{
	...
	if( dwFlushOffset > 1000 )
	{
		...
		// Send Count
		if( m_minSendCountPerSec > (DWORD)m_SendCount || m_minSendCountPerSec == 0 )
			m_minSendCountPerSec = m_SendCount;

		if( m_maxSendCountPerSec < (DWORD)m_SendCount )
			m_maxSendCountPerSec = m_SendCount;

		m_SendCount = 0;

		// SendBytes
		if( m_minSendBytesPerSec > (DWORD)m_SendBytes || m_minSendBytesPerSec == 0 )
			m_minSendBytesPerSec = m_SendBytes;

		if( m_maxSendBytesPerSec < (DWORD)m_SendBytes )
			m_maxSendBytesPerSec = m_SendBytes;

		m_SendBytes = 0;

		// Recv Count
		if( m_minRecvCountPerSec > (DWORD)m_RecvCount || m_minRecvCountPerSec == 0 )
			m_minRecvCountPerSec = m_RecvCount;

		if( m_maxRecvCountPerSec < (DWORD)m_RecvCount )
			m_maxRecvCountPerSec = m_RecvCount;

		m_RecvCount = 0;

		// RecvBytes
		if( m_minRecvBytesPerSec > (DWORD)m_RecvBytes || m_minRecvBytesPerSec == 0 )
			m_minRecvBytesPerSec = m_RecvBytes;

		if( m_maxRecvBytesPerSec < (DWORD)m_RecvBytes )
					...
					for( int i = 0; i < m_vecSocketSize; ++i )
					{
						...
						if( pSocket != NULL && pSocket->m_emStatus == EM_SOCKET_CONNECTED && pSocket->m_bConnectOnce != false && pSocket->m_emMode != EM_SOCKET_MODE_TELNET )
						{
							...
							// Check if we did kick lagger
								...
								if( (DWORD)pSocket->m_SendQueueSize > m_dwSendBuffLimit )
								{								
									char szIP[256];
									ConvertIPtoString( pSocket->m_dwIP, szIP );
