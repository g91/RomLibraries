	...
	//	_wcTempBufList.push_back( NULL );
		...
		else if(IDData->AddressFlag == ENUM_Indirect_Address)
		{
			...
			case ENUM_Type_FloatPoint:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_FLOAT  , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , SQL_C_DEFAULT  , NULL );
				break;

			case ENUM_Type_IntPoint:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_LONG   , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , SQL_C_DEFAULT  , NULL );
				break;

			case ENUM_Type_ShortPoint:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_SHORT  , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , SQL_C_DEFAULT  , NULL );
				break;

			case ENUM_Type_CharString:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_CHAR   , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , IDData->Size  , NULL );
				break;
			...
			case ENUM_Type_WCharStringBase:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_WCHAR   , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , IDData->Size  , NULL );
				break;


			case  ENUM_Type_BinaryData:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_BINARY , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , IDData->Size  , NULL );
				break;

			case ENUM_Type_BitPoint:
				bindResult = Proxy_SQLBindCol( _sqlBase->Gethstmt() , i , SQL_C_BIT    , (SQLPOINTER)(IDData->Point + UINT64(&OutData)) , SQL_C_DEFAULT  , NULL );
				break;
