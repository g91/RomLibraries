...
#define _MAX_BUFF_COUNT_			70			//最多可以上的Buff數量
...
struct NPCHateList
{
    ...
    NPCHateStruct*     GetMaxHit( )                  //找出打血最多的人
    {
		...
		for( int i = 0 ; i < Hate.Size() ; i++ )
		{
			if( Hate[i].PartyID <=0 )
				continue;
			PartyHitPointMap[ Hate[i].PartyID ] += (int)Hate[i].HitPoint;
		}
        ...
        for( int i = 0 ; i < Hate.Size() ; i++ )
        {
			int HitPoint = (int)Hate[i].HitPoint;
			if( Hate[i].PartyID > 0 )
			{
	...
	NPCHateStruct*     GetMaxHate( )                  //找出最恨的人
	{
		...
		for( int i = 0 ; i < Hate.Size() ; i++ )
		{
			...
			{
				MaxHate   = (int)Hate[i].TotalHatePoint();
				MaxHateID = i;
			}
		}
...
struct	GlobalServerDBInfoStruct
{
	...
	float	ExpRate;
	float	RelationExpRate;
	float	QuestExpRate;
	float	DropTreasureRate;
	float	TpRate;
	float	QuestTpRate;
	float	MoneyRate;
	float	QuestMoneyRate;
	float	PK_DotRate;
	float	PK_DamageRate;
	int		LearnMagicID;
...
enum	GameMessageEventENUM
{
	...
	EM_GameMessageEvent_Item_DisableTrade				,	 
	...
	EM_GameMessageEvent_Item_TargetDisableTrade			,	 
	...
	//社群
	EM_GameMessageEvent_Socal_AddFriendNoPeople		= 600,	 
	EM_GameMessageEvent_Socal_AddFriendNoOwner		,		 
	...
	EM_GameMessageEvent_Role_Untrust				,		 
...
enum CheckAddRuneResultENUM
{
	...
	EM_CheckAddRuneResult_TargetItemErr		,
	EM_CheckAddRuneResult_NotRune			,
	EM_CheckAddRuneResult_RuneVolumeErr		,
	EM_CheckAddRuneResult_WeaponErr			,
	EM_CheckAddRuneResult_ArmorErr			,
	EM_CheckAddRuneResult_Unknow			,
	EM_CheckAddRuneResult_RuneDuplicate		,
	EM_CheckAddRuneResult_RuneFull			,
	EM_CheckAddRuneResult_LevelError		,
};
//-------------------------------------------------------------------------------------
enum CheckEqRefineResultENUM
{
	EM_CheckEqRefineResult_OK					,	//OK
	EM_CheckEqRefineResult_DataErr				,
	EM_CheckEqRefineResult_MaxLevel				,
	EM_CheckEqRefineResult_NotWeapon			,
	EM_CheckEqRefineResult_NotArmor				,
	EM_CheckEqRefineResult_NotJewels			,
	EM_CheckEqRefineResult_ItemLimitMinLevel	,
	EM_CheckEqRefineResult_ItemLimitMaxLevel	,
	EM_CheckEqRefineResult_NotOrnament			,
	EM_CheckEqRefineResult_CantRefine			,
};
//-------------------------------------------------------------------------------------
enum ChecFixEqResultENUM
{
	EM_CheckFixEqResult_OK				,
	EM_CheckFixEqResult_DataErr			,
	EM_CheckFixEqResult_NotFix			,
	EM_CheckFixEqResult_NotEQ			,
	EM_CheckFixEqResult_NewOne			,
...
struct HouseBaseDBStruct
{
	...
	StaticFlag<_MAX_HOUSES_SHOPFUNCTION_COUNT_>	ShopFunction;		 
