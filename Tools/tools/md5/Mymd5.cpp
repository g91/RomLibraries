...
const char* MyMD5Class::_ComputeByteHash(const unsigned char* data, const char* XorStr, int size)
{
	::Md5Initialize();
	...
	if (XorStrLen != 0)
	{
		for (int i = 0; i < nLength; i++)
		{
			pBuffer[i] = data[i] ^ XorStr[i % XorStrLen];
		}
	}
	else
	{
		memcpy(pBuffer, data, nLength);
	}
...
const char* MyMD5Class::ComputeByteHash(const unsigned char* data, const char* XorStr, int size)
{
	g_CritSect()->Enter();
	const char* RetStr = _ComputeByteHash(data, XorStr, size);
	g_CritSect()->Leave();
	return RetStr;
}