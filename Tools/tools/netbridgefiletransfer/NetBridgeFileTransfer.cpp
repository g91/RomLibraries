	...
	case EM_CrcFileMode_Read:
		{
			...
			if (m_Stream.is_open())
			{
				...
				m_FileSize = (unsigned long)m_Stream.tellg();
...
int MyCrcFile::Read(unsigned long size, unsigned long dwBufferSize, void* pBuffer)
{
	...
	if (m_Mode == EM_CrcFileMode_Read)
	{
		if (dwBufferSize >= size)
		{
			if (!m_Stream.eof())
			{
				m_Offset = (unsigned long)m_Stream.tellg();
...
bool NetBridgeFileSender::BeginFile()
{
	...
	if (!m_Tasks.empty())
	{
		...
		if (Result == true)
		{
			...
			{
				m_OnBeginFile(this, m_CurrentTask.GUID,  m_CurrentTask.LocalFileName.c_str(), FileSize, (int)m_CompletedTasks.size(), int(m_Tasks.size() + m_CompletedTasks.size() + 1));
			}
		}
	}
