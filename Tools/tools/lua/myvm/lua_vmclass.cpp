...
void	LUA_VMClass::LoadFile(string FileName)
{
	...
	//std::cout << "LUA_VMClass::LoadFile " << FileName.c_str() << " result:" << res << std::endl;
}
...
void	LUA_VMClass::Process(int TimePass)
{
	...
	while (1)
	{
		...
		case PlotEnd:
		{
			...
			Print(LV1, "LUA_VMClass", "L_StateID = %d end\n", _St->ProcReg->L_StateID);
		...
		case PlotBeginCall:
		{
			...
			if (ret != 0)
			{
				...
				Print(LV1, "LUA_VMClass", "L_StateID = %d call\n", _St->ProcReg->L_StateID);
				//把此劇情移出
		...
		case PlotRun:
		{
			...
			else
			{
				...
				{
					//printf("L_StateID = %d 結束\n",_St->ProcReg->L_StateID);				
					Print(LV1, "LUA_VMClass", "L_StateID = %d run\n", _St->ProcReg->L_StateID);
					//把此劇情移出
					LUA_VMClass* OwnerVM = GetVMUser(_St->ProcReg->ID);
		...
		case PlotMultiRun:
		{
			...
			else
			{
				...
				Print(LV1, "LUA_VMClass", "L_StateID = %d multirun\n", _St->ProcReg->L_StateID);
				_FreeVM(_St->ProcReg->L_StateID);
				_FreePlotReg(_St->ProcReg);
				break;
			}
...
vector<string>* LUA_VMClass::_AllPathList(string Path)
{
	...
	while (OpenPath.size())
	{
		...
		if ((hFile = (UINT64)_findfirst((char*)Findmask.c_str(), &c_file)) != -1L)
		{
