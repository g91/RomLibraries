...
void    RoleDataEx::Net_FixItemInfo_Body( int Pos )
{
    ...
    if( Item != NULL )
	{
		...
		Net_BillingNotify2( Item->OrgObjID );
	}
}
...
void RoleDataEx::Net_BillingNotify2( int OrgID )
{
	int PhiriusCoinID = g_ObjectData->GetSysKeyValue( "SYS_PRICETYPE_ICONID_03" );

	if ((OrgID > 0) && (PhiriusCoinID != OrgID))
	{
		return;
	}

	int PhiriusCoin = CalBodyItemCount(PhiriusCoinID);

	Net_BillingChild::SB_MoneyInfo(Account_ID(), DBID(), PlayerBaseData->Money_Bonus, PhiriusCoin);
}
