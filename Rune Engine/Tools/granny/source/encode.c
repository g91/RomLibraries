...
/*
#include <stdio.h>
static void dump_plane( int plane, int num, S16 const * p, U32 pixel_pitch, U32 width, U32 height )
{
  FILE *f;
  U32 x, y;
  char fn[32];
    ...
    while ( lit_len )
    {
      ...
      if ( w <= 1 )
      {
        ...
        if ( w )
        {
          ...
          context = GET_BIT_LEVEL( ( ( ( U32 ) radabs( prev * 2 ) + radabs ( above_left ) + radabs( above ) ) * qlevel ) / 4 );
      ...
      else
      {
        ...
        context  = GET_BIT_LEVEL( ( ( ( U32 ) radabs( prev ) + radabs ( above_left ) + radabs( above ) + radabs( above_right ) ) * qlevel ) / 4 );
  ...
  while ( 1 )
  {
    ...
    while( lit_len )
    {
      if ( w <= 1 )
      {
        ...
        {
          INTADDR cur;
          U32 context = GET_BIT_LEVEL( ( ( U32 ) radabs( prev * 2 ) + radabs ( above_left ) + radabs( above ) ) / 4 );
      ...
      else
      {
        ...
        U32 context = GET_BIT_LEVEL( ( ( U32 ) radabs( prev ) + radabs ( above_left ) + radabs( above ) + radabs( above_right ) ) / 4 );
