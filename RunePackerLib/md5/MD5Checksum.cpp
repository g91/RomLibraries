...
BOOL CMD5Checksum::GetMD5(HANDLE hFile,char* OutMD5Str,IMD5_Progress* progress)
{
	...
	//	int nLength = 0;				//number of bytes read from the file
		const int nBufferSize = 10240;	//checksum the file in blocks of 1024 bytes
		static BYTE Buffer[nBufferSize];		//buffer for data read from the file
		...
		while(1) 
		{
			ReadFile(hFile,Buffer,nBufferSize,&readBytes,NULL);
			if (readBytes>0)
			{
	...
	{
		DPF(1,_TEXT("CMD5Checksum::GetMD5: CFileException caught"));	
