...
{
	unsigned long dwSize = unsigned long(sizeof(PG_StoC_CONSOLE_OUTPUT) + strlen(pszStr));
	...
	memcpy(pPacket->Key, UNIQUE_STOC_KEY, UNIQUE_KEY_SIZE);
...
bool ControllerClient::OnRecv(unsigned long dwNetID, unsigned long dwSize, void* PG)
{
	...
	if (PG)
	{
		...
		{
			if (memcmp(pPkt->Key, GetUniqueKey(pPkt->PGID), UNIQUE_KEY_SIZE) != 0)
			{
				//Print(5, "ControllerClient::OnRecv", "Key error packet id:%d", pPkt->PGID);
				char Buff[256];
				SecureZeroMemory(Buff, sizeof(Buff));
				sprintf_s(Buff, sizeof(Buff), "Key error packet id:%d", pPkt->PGID);				
				Print(LV2, "ControllerClientEvent::OnConnectFailed", Buff);
				return Result;
			}
