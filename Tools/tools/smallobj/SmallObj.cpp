...
string FastWcharToChar(const wchar_t* wcStr)
{
	if (wcStr == NULL || wcslen(wcStr) == 0)
		return "";
	int cStrLen = WideCharToMultiByte(CP_ACP, 0, wcStr, -1, NULL, NULL, NULL, NULL);
	char* cStrTmp = NEW char[cStrLen];
	WideCharToMultiByte(CP_ACP, 0, wcStr, -1, cStrTmp, cStrLen, NULL, NULL);
	string cStr = cStrTmp;
	delete cStrTmp;
	return cStr.c_str();
}
